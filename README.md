...
### Запуск UI из консоли
На машине должен быть установлен JDK 21 или JRE 21.
Переменная окружения JAVA_HOME должна смотреть на корень jdk-21.
Для старта (*nix) необходимо открыть консоль в корне проекта и ввести команду:
```bash
./gradlew runUI
```
Для старта (win) необходимо открыть консоль в корне проекта и ввести команду:
```bash
gradlew.bat runUI
```
